...
<%
	final Logger logger = Logger.getLogger("workproposals.jsp");
...
<body>
	<jsp:include page="common-header.jsp">
		<jsp:param value="workproposals" name="item"/>
	</jsp:include>
	<div class="container-fluid">
		<div class="row">
			
			<div class="col-sm-8" id="companytable" style="min-height: 400px;">
				<br />
				<h3 class="ml-2">Work Proposals</h3>
				<jsp:include page="workproposalcard.jsp">
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			$(document).on('click','.fa-thumbs-up', function(){
				var id = $(this).attr('data-wpid');
				<%if (user != null) {%>
				$.post(
					'<%=host + docBase%>/like/<%=user.getAadharId()%>/workproposal',
					{
						proposal_id: id
					},
					function(response){
						...
						}else if(response == 'ownproposal'){
							alert('You are not allowed to upvote your own work proposal.');
			...
			$(document).on('click','.fa-thumbs-down', function(){
				...
				<%if (user != null) {%>
				$.post(
					'<%=host + docBase%>/dislike/<%=user.getAadharId()%>/workproposal',
					{
						proposal_id : id
					},
					function(response) {
						...
						if (response == 'success') {
							...
							alert('You are not allowed to downvote your own work proposal.');
						} else {
							alert('unable to process request');
						}
					});
