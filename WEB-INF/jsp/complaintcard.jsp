...
<%
	final Logger logger = Logger.getLogger("JanManch - complaincard.jsp");
	...
	int MAX_ROWS = noRow;
	int pages = 0;
	ComplaintDAO company = new ComplaintDAO();
	int totalRecords = 0;
	...
	DatabaseResource data = null;
		...
		data = company.getComplainList(col, ord, off <= 1 ? 0 : ((off - 1) * MAX_ROWS), MAX_ROWS);
		...
		try {
			...
			if (rs != null && rs.next()) {
				...
				do {
					...
					String user_aadhar_id = rs.getString("aadhar_no");
...
<div class="card" id="card<%=id %>">
	...
	<p><span style="font-weight: 500;">Subject: </span> <%=subject %></p>
	<p><span style="font-weight: 500;">Department: </span> <%=department %>(<%=complain_type %>)</p>
		
		<p><%=description %></p>
		<p>
			<span class="m-2"><i class="fa fa-thumbs-up text-success" style="cursor: pointer;" data-cmid="<%= id%>"></i><span id="cntlike<%=id %>" class="m-1">(<%= upvotes %>)</span></span> 
			<span class="m-2"><i class="fa fa-thumbs-down text-danger" style="cursor: pointer;" data-cmid="<%= id%>"></i><span id="cntdislike<%=id %>" class="m-1">(<%= downvotes %>)</span></span> 
