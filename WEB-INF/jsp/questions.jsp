...
<%
	final Logger logger = Logger.getLogger("questions.jsp");
...
<jsp:param value="questions" name="item"/>
</jsp:include>
<div class="container-fluid">
	<br />
	<h3 class="ml-2"> Questions from People </h3>
	<div class="row">
		<div class="col-xs-12 col-sm-8" id="question_table" style="min-height: 400px;">
			<jsp:include page="questioncard.jsp">
		...
		<div class="col-sm-2">
				<div class="panel panel-primary">
					<div class="panel-heading"><a style="background-color: rgb(2, 117, 216); color: rgb(255, 255, 255); padding: 10px; border-radius: 5px;"href="<%= user!=null?(host+docBase+"/user/"+user.getAadharId()+"/submitquery"):"login" %>">Submit Review</a></div>
				</div>
			</div>
	</div>
</div>
<jsp:include page="common-footer.jsp"></jsp:include>
	...
	$(document).ready(function(){
		$(document).on('click','.fa-thumbs-up', function(){
			...
			<% if(user!=null){%>
			$.post(
				'<%=host+docBase%>/like/<%= user.getAadharId()%>/query',
				{
					query_id: id
				},
				function(response){
					...
					}else if(response == 'ownquery'){
						alert('You are not allowed to upvote your own query.');
		...
		$(document).on('click','.fa-thumbs-down', function(){
			...
			<% if(user!=null){%>
			$.post(
				'<%=host+docBase%>/dislike/<%= user.getAadharId()%>/query',
				{
					query_id: id
				},
				function(response){
					...
					if(response == 'success'){
						...
						alert('You are not allowed to downvote your own query.');
					}else{
						alert('unable to process request');
					}
				}
         ...
         function fetchcompany(column, order, pageno, numRows, search){
				...
				$.post(
						...
						function(response){
							$('#question_table').html(response.trim());
