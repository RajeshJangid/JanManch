...
<%
	final Logger logger = Logger.getLogger("complaints.jsp");
...
<jsp:param value="complaints" name="item"/>
</jsp:include>
<div class="container-fluid">
	<br />
	<h3 class="ml-2"> Complaints from People </h3>
	
	<div class="row">
		<div class="col-xs-12 col-sm-8" id="complaincard" style="min-height: 400px;">
			<jsp:include page="complaintcard.jsp">
		...
		<div class="col-sm-2">
				<div class="panel panel-primary">
					<div class="panel-heading"><a style="background-color: rgb(2, 117, 216); color: rgb(255, 255, 255); padding: 10px; border-radius: 5px;"href="<%= user!=null?(host+docBase+"/user/"+user.getAadharId()+"/filecomplaint"):"login" %>">Submit Complaint</a></div>
				</div>
			</div>
	</div>
    ...
    <script type="text/javascript">
    	...
    	$(document).ready(function(){
    		$(document).on('click','.fa-thumbs-up', function(){
    			var id = $(this).attr('data-cmid');
    			<% if(user!=null){%>
    			$.post(
    				'<%=host+docBase%>/like/<%= user.getAadharId()%>/complaint',
    				{
    					complaint_id: id
    				},
    				function(response){
    					...
    					}else if(response == 'owncomplaint'){
    						alert('You are not allowed to upvote your own complaint.');
			...
			$(document).on('click','.fa-thumbs-down', function(){
    			...
    			<% if(user!=null){%>
    			$.post(
    				'<%=host+docBase%>/dislike/<%= user.getAadharId()%>/complaint',
    				{
    					complaint_id: id
    				},
    				function(response){
    					...
    					if(response == 'success'){
    						...
    						alert('You are not allowed to downvote your own complaint.');
    					}else{
    						alert('unable to process request');
    					}
    				}
         ...
         function fetchcompany(column, order, pageno, numRows, search){
				...
				$.post(
						...
						function(response){
							$('#complaincard').html(response.trim());
