...
<%
	...
	if (user != null) {
		...
		data = company.getQueryListForuser(user.getAadharId(), col, ord, off <= 1 ? 0 : ((off - 1) * MAX_ROWS), MAX_ROWS);
...
<div class="card" id="card_<%=id%>">
	<div class="col-sm-12 mt-1 mb-1">
		...
		<p><%=description%></p>
		<p>
			<span class="m-2"><i class="fa fa-thumbs-up text-success" style="cursor: pointer;" data-qid="<%=id%>"></i><span id="cntlike<%=id%>" class="m-1">(<%=upvotes%>)
			</span></span> <span class="m-2"><i class="fa fa-thumbs-down text-danger" style="cursor: pointer;" data-qid="<%=id%>"></i><span id="cntdislike<%=id%>" class="m-1">(<%=downvotes%>)
