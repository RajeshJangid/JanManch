...
<%
	...
	data = company.getComplainListForMLA(user.getPincode(),col, ord, off <= 1 ? 0 : ((off - 1) * MAX_ROWS), MAX_ROWS);
...
<div class="card" id="card<%=id%>">
	<div class="col-sm-12 mt-1 mb-1">
		...
		<p>
			...
			<%=department%>(<%=complain_type%>)
		</p>
		<p><%=description%></p>
		<p>
			<span class="m-2"><i class="fa fa-thumbs-up text-success" style="cursor: pointer;" data-cmid="<%=id%>"></i><span id="cntlike<%=id%>" class="m-1">(<%=upvotes%>)
			</span></span> <span class="m-2"><i class="fa fa-thumbs-down text-danger" style="cursor: pointer;" data-cmid="<%=id%>"></i><span id="cntdislike<%=id%>" class="m-1">(<%=downvotes%>)
			</span></span> <span class="m-2 text-muted pull-right" style="font-size: .8em"><%=publish_date%></span>
			<% if(user.getType().equals("M")){ %>
			<button class="btn btn-danger" onclick="">Mark as Viewed</button>
			<%}%>
		</p>
	</div>
</div>
